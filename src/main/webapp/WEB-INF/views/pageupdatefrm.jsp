...
<body>
	<form name="pageupdatefrm" method="post" action="pageUpdate" id="frm">
	...
	<%-- 	<select name="themeNo" id="themeNo" class="form-control">
			<c:forEach var="page" items="${pList}">
				<option value="${page.themeNo}"></option>
			</c:forEach>
		</select> --%>
		<table id=table> 
		<c:forEach var="page" items="${pList}">
		<input type="hidden" name="pageNo" value="${page.pageNo}">
		<input type="hidden" name="themeNo" value="${page.themeNo}">
		<div>
			<textarea name="pagecontent" id="pagecontent" class="form-control" 
				placeholder="내용을 입력해주세요">${page.pagecontent }</textarea>
				   <script>
		...
		<div id="pp">
		  작성일:<input type="hidden" name="writedate" id="writedate" value="<%=sf.format(nowTime)%>"><%=sf.format(nowTime)%><br>
			...
			<button type="submit" class="btn btn-primary" id="wri">글수정</button>
		</div>
